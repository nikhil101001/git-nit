    ...
    cb({
      responseHeaders: {
        ...
        'Content-Security-Policy': [
          // worker-src/blob: for Monaco's editor web worker (M2 conflict editor).
          "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src 'self'; worker-src 'self' blob:"
        ]
      }
    })
  })
